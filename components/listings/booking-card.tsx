  ...
  if (isOwner) {
    return (
      <div className="rounded-xl border border-border bg-card p-6">
        <h3 className="text-base font-semibold text-foreground">{t("booking.card.manage_listing")}</h3>
        ...
        <div className="mt-4 space-y-2">
          ...
          >
            ...
            {t("booking.card.go_to_your_listings")}
          </Link>
        </div>
      </div>
    )
    ...
    <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
      ...
      <div className="mb-6">
        ...
        <div className="mb-2 grid grid-cols-7 text-center text-xs font-medium text-muted-foreground">
          <div>{t("booking.card.weekday_mon")}</div>
          <div>{t("booking.card.weekday_tue")}</div>
          <div>{t("booking.card.weekday_wed")}</div>
          <div>{t("booking.card.weekday_thu")}</div>
          <div>{t("booking.card.weekday_fri")}</div>
          <div>{t("booking.card.weekday_sat")}</div>
          <div>{t("booking.card.weekday_sun")}</div>
        </div>
        ...
        <div className="mb-4 rounded-lg bg-muted p-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium text-foreground">
              ...
              {endDate && ` - ${format(endDate, "d MMM", { locale: dateLocale })}`}
            </span>
            {totalDays > 0 && (
      ...
      {totalDays > 0 && (
        <div className="mb-4 space-y-2 border-t border-border pt-4 text-sm">
          ...
          <div className="flex justify-between border-t border-border pt-2 font-semibold">
            <span className="text-foreground">{t("booking.card.total_to_pay")}</span>
          ...
          {deposit > 0 && (
            <div className="rounded-lg bg-muted p-2 text-xs text-muted-foreground">
              {t("booking.card.deposit_separate")} {formatPrice(deposit, listing.currency_code)}. {t("booking.card.total_authorized")}
